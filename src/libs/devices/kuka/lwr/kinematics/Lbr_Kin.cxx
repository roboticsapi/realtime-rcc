...
namespace kuka_lwr
{
	void Lbr_Kin::lbrKin(double jnt1, double jnt2, double jnt3, double jnt4, double jnt5, double jnt6, double jnt7,
			KDL::Frame& pos, double& alpha, double l0, double l1, double l2, double l3, double l4,
			double l5, double l6, double l7)
	{
		double L0 = l0 + l1, L1 = l2 + l3, L2 = l4 + l5, L3 = l6 + l7;
		double j0 = jnt1, j1 = jnt2, j2 = jnt3, j3 = jnt4, j4 = jnt5, j5 = jnt6, j6 = jnt7;
        ...
        // build matrix
		pos = KDL::Frame(
				KDL::Rotation(M11, M21, M31, M12, M22, M32, M13, M23, M33),
				KDL::Vector(M41, M42, M43)
		);
		...
		if (fabs(j3) < 1e-5) {
			...
			double hwpLen = sqrt(((hwpX*hwpX) + (hwpY*hwpY)) + (hwpZ*hwpZ));
			...
			// calculate alpha
			double cos = (((zeroX*dirX) + (zeroY*dirY)) + (zeroZ*dirZ))/(sqrt(zeroLen)*sqrt(dirLen));
			...
			alpha = (dir < 0 ? -1 : 1) * acos(cos);
		}
	}

	void Lbr_Kin::lbrInvKin(KDL::Frame pos, double alpha, double& jnt1, double& jnt2, double& jnt3, double& jnt4,
			double& jnt5, double& jnt6, double& jnt7, double l0, double l1, double l2, double l3, double l4,
			double l5, double l6, double l7)
	{
		double L0 = l0 + l1, L1 = l2 + l3, L2 = l4 + l5, L3 = l6 + l7, PI = 4 * atan(1.0);

		double r0 = 0, r1 = 0, r2 = 0, r3 = 0, r4 = 0, r5 = 0, r6 = 0;
		double j0 = jnt1, j1 = jnt2, j2 = jnt3, j3 = jnt4, j4 = jnt5, j5 = jnt6, j6 = jnt7;

		double m11 = pos.M.UnitX().x(); //pos[0, 0];
		double m21 = pos.M.UnitY().x(); //pos[0, 1];
		double m31 = pos.M.UnitZ().x(); //pos[0, 2];
		double m41 = pos.p.x(); //pos[0, 3];
		double m12 = pos.M.UnitX().y(); //pos[1, 0];
		double m22 = pos.M.UnitY().y(); //pos[1, 1];
		double m32 = pos.M.UnitZ().y(); //pos[1, 2];
		double m42 = pos.p.y(); //pos[1, 3];
		double m13 = pos.M.UnitX().z(); //pos[2, 0];
		double m23 = pos.M.UnitY().z(); //pos[2, 1];
		double m33 = pos.M.UnitZ().z(); //pos[2, 2];
		double m43 = pos.p.z(); //pos[2, 3];
		...
		double a1 = atan2(hwpY, hwpX);
		double gamma = atan2(sqrt(hwpX * hwpX + hwpY * hwpY), hwpZ);
		double a = L1, b = L2, c = sqrt(hwpX * hwpX + hwpY * hwpY + hwpZ * hwpZ);

		if (c > a + b && c < a + b + 0.001) c = a + b;
		double a2 = acos((-b * b + a * a + c * c) / (2 * a * c)) - gamma;
		double a3 = PI - acos((-c * c + a * a + b * b) / (2 * a * b));
		...
		else
		{
			...
			double rotEllY = ((L1 * (s1a * c0a) * ((hx * hy - (ca * (hx * hy))) + (sa * hz))) +
							  (L1 * (s1a * s0a) * (hy * hy + (ca * (1.0 - hy * hy))))) + (L1 * c1a * ((hy * hz - (ca * (hy * hz))) - (sa * hx)));
			double rotEllZ = ((L1 * (s1a * c0a) * ((hx * hz - (ca * (hx * hz))) - (sa * hy))) +
							  (L1 * (s1a * s0a) * ((hy * hz - (ca * (hy * hz))) + (sa * hx)))) + (L1 * c1a * (hz * hz + (ca * (1.0 - hz * hz))));

			// calculate angles for ellbow
			r0 = atan2(rotEllY, rotEllX);
			r1 = -atan2(sqrt(rotEllX * rotEllX + rotEllY * rotEllY), rotEllZ);
			...
			// we want to know how far we have to turn the "Up" vector at the ellbow around "Backward"
			...
			double ellCrossHwLen = ((ellCrossHwX * ellCrossHwX) + (ellCrossHwY * ellCrossHwY)) + (ellCrossHwZ * ellCrossHwZ);
			double cos = (((ellCrossHwX * -s0a) + (ellCrossHwY * c0a))) /
						 (sqrt(ellCrossHwLen) * sqrt(ellYLen));
		...
		while (r0 < jointlimit)
		{
			if (r0 > -jointlimit && fabs(r0 - j0) + fabs(r1 - j1) < mindist)
			{
				rr0 = r0;
				rr1 = r1;
				rr2 = r2;
				mindist = fabs(r0 - j0) + fabs(r1 - j1);
			}
			r0 += PI;
			r1 = -r1;
			r2 += PI;
		...
		// select j3/j4
		mindist = 999, rr0 = r2, rr1 = r3, rr2 = r4;
		while (r2 > -jointlimit)
		{
			r2 -= PI;
			r3 = -r3;
		...
		while (r2 < jointlimit)
		{
			if (r2 > -jointlimit && fabs(r2 - j2) + fabs(r3 - j3) < mindist)
			{
				rr0 = r2;
				rr1 = r3;
				rr2 = r4;
				mindist = fabs(r2 - j2) + fabs(r3 - j3);
			}
			r2 += PI;
			r3 = -r3;
			r4 += PI;
		...
		r4 = atan2(rotM32, rotM31);
		r5 = -atan2(sqrt(rotM31 * rotM31 + rotM32 * rotM32), rotM33);
		r6 = atan2(rotM23, -rotM13);

		// select j5/j6
		mindist = 999, rr0 = r4, rr1 = r5, rr2 = r6;
		while (r4 > -jointlimit)
		{
			r4 -= PI;
			r5 = -r5;
		...
		while (r4 < jointlimit)
		{
			...
			while (r6 < -jointlimit)
				r6 += 2 * PI;
			if (r4 > -jointlimit && (fabs(r4 - j4) + fabs(r5 - j5) + fabs(r6 - j6) < mindist))
			{
				if(r6 < jointlimit) {
					rr0 = r4;
					rr1 = r5;
					rr2 = r6;
					mindist = fabs(r4 - j4) + fabs(r5 - j5) + fabs(r6 - j6);
				} else if(mindist==999) {
					rr0 = r4;
					rr1 = r5;
					rr2 = r6;
		...
		if(mindist != 999) {
			...
			r5 = rr1;
			r6 = rr2;
			if(fabs(r2) > PI-jointlimit && fabs(r0) > PI-jointlimit) {
				double d2 = r2>0?r2-PI:r2+PI;
				double d0 = r0>0?r0-PI:r0+PI;
				if(fabs(d2-j2)>fabs(d0-j0)) {
					// use j0
					r0 = d0; r1 = -r1; r3 = -r3; r5 = -r5; r6 -= PI;
				} else {
					// use j2
					r2 = d2; r3 = -r3; r5 = -r5; r6 -= PI;
				}
			} else if(fabs(r2) > PI-jointlimit) {
				double d2 = r2>0?r2-PI:r2+PI;
				// use j2
				r2 = d2; r3 = -r3; r5 = -r5; r6 -= PI;
			} else if(fabs(r0) > PI-jointlimit) {
				double d0 = r0>0?r0-PI:r0+PI;
				// use j0
				r0 = d0; r1 = -r1; r3 = -r3; r5 = -r5; r6 -= PI;
			} else {
	...
	void Lbr_Kin::lbrVelKin(double jnt1, double jnt2, double jnt3, double jnt4, double jnt5, double jnt6, double jnt7,
			double vel1, double vel2, double vel3, double vel4, double vel5, double vel6, double vel7,
			KDL::Twist& vel, double& velalpha, double l0, double l1, double l2, double l3, double l4,
			double l5, double l6, double l7)
	{
		KDL::Frame pos1, pos2;
		double alpha1, alpha2;
		double dt = 0.001;
		lbrKin(jnt1, jnt2, jnt3, jnt4, jnt5, jnt6, jnt7, pos1, alpha1, l0, l1, l2, l3, l4, l5, l6, l7);
		lbrKin(jnt1 + vel1*dt, jnt2 + vel2*dt, jnt3 + vel3*dt, jnt4 + vel4*dt, jnt5 + vel5*dt, jnt6 + vel6*dt, jnt7 + vel7*dt, pos2, alpha2, l0, l1, l2, l3, l4, l5, l6, l7);

		vel = KDL::diff(pos1, pos2, dt);
		velalpha = (alpha2-alpha1)/dt;
	}

	/**
	 * Lbr_Kin cannot be instantiated, contains only static methods
	 */
	Lbr_Kin::Lbr_Kin()
	{
	}
