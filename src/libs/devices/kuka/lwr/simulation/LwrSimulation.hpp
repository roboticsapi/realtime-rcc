...
namespace kuka_lwr
{
	const std::string dev_lwr_joint_sim = "kuka_lwr_sim";

	class LwrSimulation: virtual public LwrDevice,
			virtual public robotarm::RobotArmDriverSimulation,
			virtual public IO::IODeviceSimulation
	{
	public:
		LwrSimulation(std::string name, RPI::parameter_t parameters);
		virtual ~LwrSimulation();

		static LwrSimulation* createDevice(std::string name, RPI::parameter_t parameters);
