...
#include "analyticalinvkin.hpp"
#include <stdio.h>
#include <kdl/frames.hpp>

namespace kuka_youbot
{
#define DEG_TO_RAD(x) ((x) * KDL::PI / 180.0)
	...
	ArmAnalyticalInverseKinematics::ArmAnalyticalInverseKinematics()
	{
		...
		_min_angles.push_back(DEG_TO_RAD(-169));
		_min_angles.push_back(DEG_TO_RAD(-65));
		_min_angles.push_back(DEG_TO_RAD(-151));
		_min_angles.push_back(DEG_TO_RAD(-102.5));
		_min_angles.push_back(DEG_TO_RAD(-165));

		_max_angles.push_back(DEG_TO_RAD(169));
		_max_angles.push_back(DEG_TO_RAD(90));
		_max_angles.push_back(DEG_TO_RAD(146));
		_max_angles.push_back(DEG_TO_RAD(102.5));
		_max_angles.push_back(DEG_TO_RAD(165));
	}
	...
	KDL::JntArray ArmAnalyticalInverseKinematics::ik(const KDL::Frame& g0, bool offset_joint_1, bool offset_joint_3,
			bool offset_joint_5)
	{
...
//		double d = 0.13;
		...
		for (int i = 0; i < 3; i++)
		{
			for (int j = 0; j < 3; j++)
			{
				if (fabs(g2_proj.M(i, j)) < 0.000001)
					g2_proj.M(i, j) = 0;
			}
		}
		...
		if (j3_cos > ALMOST_PLUS_ONE)
			j3 = 0.0;
		else if (j3_cos < ALMOST_MINUS_ONE)
			j3 = KDL::PI;
		else
			j3 = atan2(sqrt(1 - (j3_cos * j3_cos)), j3_cos);
		if (offset_joint_3)
			j3 = -j3;
		...
		{
			if (j3 >= 0)
				j2 = -atan2(p2.z(), p2.x()) - atan2(l3 * sin(j3), l2 + l3 * cos(j3));
			else
				j2 = -atan2(p2.z(), p2.x()) + atan2(l3 * sin(j3), l2 + l3 * cos(j3));
		} else
		{
...
//		printf("Configuration without offsets: %f, %f, %f, %f, %f", j1, j2, j3, j4, j5);
//		printf("Configuration with offsets: %f, %f, %f, %f, %f", solution(0), solution(1), solution(2), solution(3),
//				solution(4));
	...
	KDL::Frame ArmAnalyticalInverseKinematics::ProjectGoalOrientationIntoArmSubspace(const KDL::Frame &goal) const
	{
		...
		KDL::Vector y_t_hat_tick = (cos_theta * y_t_hat) + (sin_theta * (k_hat * y_t_hat))
				+ (1 - cos_theta) * (KDL::dot(k_hat, y_t_hat)) * k_hat; // k_hat * y_t_hat is cross product
