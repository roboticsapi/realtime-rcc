...
namespace RPI
{
	...
	public:
		...
		std::map<std::string,std::map<std::string,std::string>> implementedDeviceInterfaces;
		RTT::os::TimeService::nsecs lastUpdated;
	...
	protected:
		...
		void notifyInterfaceDataChanged();
	...
	public:
		...
		/**
		 * \brief Update the device interfaces for the device
		 * \param name Name of the device to be updated
		 * \param device Instance of device
		 *
		 * The device's interfaces are updated, and the last update timestamp is adjusted
		 */
		virtual void updateInterfaces(const std::string& name, Device* device) = 0;
