...
int ORO_main(int argc, char** argv)
{
	...
	if (RegistryImpl::getRegistry()->start())
	{
		if (headless)
		{
		...
		} else
		{
			...
			while (!exit)
			{
				...
				} else if (inp == "n" || inp == "nets")
				{
					...
					{
						unsigned int len = Registry::getRegistry()->getNet(net)->getName().length();
						if (len > col1)
							col1 = len;
					}
					...
					for (const auto& net : nets)
					{
						auto netptr = Registry::getRegistry()->getNet(net);
						if(!netptr)
							continue;

						string netname = netptr->getName();

						string desc = netptr->getDescription();

						string status = HTTPServer_state_text(netptr->getNetState());
