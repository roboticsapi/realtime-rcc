...
namespace RPI
{
	...
	string NetsHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		...
		if (method == "POST")
		{
			...
			if (xml != "" || dio != "")
			{
				...
				if (loadsuccess)
				{
					string netname = Registry::getRegistry()->getNetName(netid);
		...
		} else
		{
			...
			{
				string netname = Registry::getRegistry()->getNetName(*it);
				if (netname == "INVALID")
					continue;
				NetState state = Registry::getRegistry()->getNetState(*it);
				if(state == Unloading)
					state = Terminated;
				string desc = Registry::getRegistry()->getNetDescription(*it);

				string status = HTTPServer_state_text(Registry::getRegistry()->getNetState(*it));
	...
	string DevicesHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		...
		} else
		{
			...
			for (devicelist_t::iterator it = devices.begin(); it != devices.end(); ++it)
			{
				...
				for(std::list<std::string>::const_iterator it2 = it->implementedInterfaces.begin(); it2 != it->implementedInterfaces.end();++it2)
				{
					ret += "<interface name=\"" + (*it2) + "\"/>\r\n";
				}
	...
	string NetHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		...
		Net_ID_t netid = Registry::getRegistry()->getNetForName(netname);
		if (netid == NET_INVALID)
			return "";

		map<string, string> commInProperties = Registry::getRegistry()->getNetInProperties(netid);
		Netcomm_Map_t commOutProperties = Registry::getRegistry()->getNetOutProperties(netid);
		...
		if (method == "POST")
		{
			...
			if (action == "START")
			{
				Registry::getRegistry()->startNet(netid);
			...
			{
				// othernetid may be NET_INVALID, in this case schedule will react just like start()
				Net_ID_t othernetid = Registry::getRegistry()->getNetForName(getVar(data, "takeovernet"));
			...
			if (!commWrite.empty())
				Registry::getRegistry()->writeNetInProperties(netid, commWrite);
		...
		ret << "HTTP/1.1 200 OK\r\n" << "Content-Type: text/xml\r\n" << "\r\n"
				...
				Registry::getRegistry()->getNetState(netid)) << "\" desc=\"" << HTML_escape_attrib(Registry::getRegistry()->getNetDescription(netid)) << "\">\r\n";
		...
		list<NetErrorState> ne = Registry::getRegistry()->getNetErrorState(netid);
	...
	string NetXmlHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		...
		stringstream ret;
		string xml = Registry::getRegistry()->getNetXml(netid);
		ret << "HTTP/1.1 200 OK\r\n" << "Content-Type: text/xml\r\n" << "\r\n"
				<< "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n" << xml << "\r\n";
	...
	string NetDioHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		...
		stringstream ret;
		string dio = Registry::getRegistry()->getNetDio(netid);
	...
	string NetDebugHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		...
		stringstream ret;
		string debug = Registry::getRegistry()->getNetDebug(netid);
		ret << "HTTP/1.1 200 OK\r\n" << "Content-Type: text/plain\r\n" << "\r\n" << debug;
	...
	bool NetDebugStreamHandler::handleRequest(vector<string> path, string method, string data, string get, mg_connection* conn)
	{
		...
		while(true) {
			string debug = Registry::getRegistry()->getNetDebug(netid, cycle);
	...
	string NetStatsHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		...
		ret << "HTTP/1.1 200 OK\r\n"
			<< "Content-Type: text/xml\r\n" << "\r\n"
			<< "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\r\n"
			<< "<net min=\"" << Registry::getRegistry()->getNetMinTime(netid)/1000000.0
			<< "\" max=\"" << Registry::getRegistry()->getNetMaxTime(netid)/1000000.0
			<< "\" mincycle=\"" << Registry::getRegistry()->getNetMinCycleTime(netid)/1000000.0
			<< "\" maxcycle=\"" << Registry::getRegistry()->getNetMaxCycleTime(netid)/1000000.0
			<< "\">\r\n";

		map<string,RTT::os::TimeService::nsecs> min = Registry::getRegistry()->getNetBlockMinTime(netid),
				max = Registry::getRegistry()->getNetBlockMaxTime(netid),
				total = Registry::getRegistry()->getNetBlockTotalTime(netid);
	...
	string ModuleJavaSourceHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		ModuleFactoryMap* modules = ExtensionLoader::getInstance()->getModuleFactoryMap();
		string name = path[1];
		string
				ret =
						string("HTTP/1.1 200 OK\r\n") + "Content-Type: text/xml\r\n" + "\r\n"
								+ "<?xml version=\"1.0\" encoding=\"UTF-8\"?><?xml-stylesheet type=\"text/xsl\" href=\"/xsl/module.javarcc.xsl\" ?>\r\n"
								+ "<module name=\"" + name + "\"";
		if (modules->find(name) != modules->end())
		{
			Module* module = (*modules)[name]->createInstance(name, 0);
	...
	string SessionHandler::handleRequest(vector<string> path, string method, string data, string get)
	{
		...
		if (method == "POST")
		{
			...
			if (action == "LOADNET")
			{
				...
				string status = "/nets/" + Registry::getRegistry()->getNetName(net) + "/";
				return string("HTTP/1.1 303 See Other\r\n") + "Location: " + status + "\r\n"
						+ "Content-Type: text/html\r\n" + "\r\n" + "<html><body><h1>See Other</h1></body></html>";
			...
			} else if (action == "UPDATE")
			{
				...
				for(vector<Net_ID_t>::iterator ni = nets.begin(); ni != nets.end(); ++ni) {
					string net = Registry::getRegistry()->getNetName(*ni);
					if(Registry::getRegistry()->getNetState(*ni) == Terminated)
						continue;

					map<string, string> commInProperties = Registry::getRegistry()->getNetInProperties(*ni);
					map<string, string> commWrite;
					for (map<string, string>::iterator it = commInProperties.begin(); it != commInProperties.end(); ++it)
					{
					...
					if (!commWrite.empty())
						Registry::getRegistry()->writeNetInProperties(*ni, commWrite);
				}
		...
		for(vector<Net_ID_t>::iterator ni = nets.begin(); ni != nets.end(); ++ni) {
			NetState state = Registry::getRegistry()->getNetState(*ni);
			if(state == Unloading) continue;
			ret << "\t<n id=\"" << Registry::getRegistry()->getNetName(*ni) << "\" s=\""
					<< HTTPServer_state_text(state) << "\">";

			Netcomm_Map_t commOutProperties = Registry::getRegistry()->getNetOutProperties(*ni);
			for (Netcomm_Map_t::iterator it = commOutProperties.begin(); it != commOutProperties.end(); ++it)
				ret << "<d k=\"" << it->first << "\">" << it->second.first << "</d>";

			list<NetErrorState> ne = Registry::getRegistry()->getNetErrorState(*ni);
			if(ne.size()>0)
