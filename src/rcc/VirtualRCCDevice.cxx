...
namespace RPI
{
	...
	VirtualRCCDevice::VirtualRCCDevice(std::string name, RPI::parameter_t parameter) :
			Device(name, parameter), RTT::TaskContext("vrcc", PreOperational), lastEStopEvent(0), lastEStopTriggered(0), isEStop(
					false), needEStop(false)
	{
		...
		dioport = this->parameters["dioport"];

		this->setActivity(new RTT::extras::PeriodicActivity(RTT::os::HighestPriority, VRCC_CYCLE_TIME));
		this->configure();
		this->start();
	}
	...
	std::string VirtualRCCDevice::getDIOPort() const
	{
		return dioport;
	}

	bool VirtualRCCDevice::configureHook()
	{
		return true;
	}

	bool VirtualRCCDevice::startHook()
	{
		return true;
	}

	void VirtualRCCDevice::updateHook()
	{
	...
	void VirtualRCCDevice::stopHook()
	{

	}

	void VirtualRCCDevice::cleanupHook()
	{
