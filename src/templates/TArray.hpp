...
namespace RPI
{
	...
	private:
		...
		long array_dimmension;

	public:
		Array() : data(), array_dimmension(0)
		{
		}

		bool copyTo(Array<T> dest) const
		{
			if(dest.getSize() != getSize()) {
				return false;
			}
			memcpy(dest.data.get(), data.get(), sizeof(T)*getSize());
			return true;
		}

		bool copyOfRange(Array<T> dest, int start)
		{
			if(start < 0 || start+dest.getSize() > getSize()) {
				return false;
			}
			memcpy(dest.data.get(), data.get()+start, sizeof(T)*dest.getSize());
			return true;
		}

		void resize(long size)
		{
			array_dimmension = size;
			data = boost::shared_array<T>(new T[array_dimmension]);
		}

		T& operator[] (long index)
		{
			if(index < 0 || index >= array_dimmension)
